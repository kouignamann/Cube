...
public class FinalShader extends ShaderObject {
    
    public FinalShader() {
        super();

        int shaderProgramId = GL20.glCreateProgram();
        int vertexShaderId = ShaderUtils.loadShader("/shaders/vertex2.glsl", GL20.GL_VERTEX_SHADER);
        int fragmentShaderId = ShaderUtils.loadShader("/shaders/fragment2.glsl", GL20.GL_FRAGMENT_SHADER);
